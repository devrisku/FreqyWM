...
def wm_verify(filename,threshold,threshold_ver, pairs, rnd, z,type=0):
    """
    ...
    for i in range(len(pairs)):
        ...
        # if list_w.get(pairs[i][0])!=None and list_w.get(pairs[i][1])!=None:
           in_msg = list_w[ind1].get_name() + " " + str(rnd)
           hash_inner = hashlib.sha256(in_msg.encode('utf-8')).hexdigest()
           out_msg = hash_inner + " " + list_w[ind2].get_name()
...
def wm_verify_hist(list_w,threshold,threshold_ver, pairs, rnd, z):
    """
    ...
    if (count*100)/len(pairs) >= threshold:
            result = 1
    #print("# of accepted pairs (%) for sampled: ", " : ",np.ceil((count*100)/len(pairs)))
    return result, (count*100)/len(pairs),non_found_total
